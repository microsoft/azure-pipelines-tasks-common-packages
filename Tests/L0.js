...
describe('General Suite', function () {
    ...
    it('Find unsupported demands', (done) => {
        this.timeout(parseInt(process.env.TASK_TEST_TIMEOUT) || 20000);
        var supportedDemands = ['AndroidSDK',
            'ant',
            'AzurePS',
            'Chef',
            'DotNetFramework',
    ...
    it('Find unsupported runsOn', (done) => {
        this.timeout(parseInt(process.env.TASK_TEST_TIMEOUT) || 20000);
        var supportedRunsOn = ['Agent', 'DeploymentGroup', 'Server', 'ServerGate'];
    ...
    it('Find invalid server Task/ServerGate', (done) => {
        ...
        var supportedServerExecutionHandlers = [
        ...
        var supportedServerGateExecutionHandlers = [
            'ServiceBus',
            'HttpRequest',
            'HttpRequestChain'
        ];
        var supportedTaskEvents = [
            'TaskAssigned',
            'TaskStarted',
            'TaskCompleted'
        ];
        var invalidTaskFound = false;
        for (var i = 0; i < taskFolders.length; i++) {
            var taskFolder = taskFolders[i];
            var taskjson = path.join(taskFolder, 'task.json');
            var task = JSON.parse(fs.readFileSync(taskjson).toString());
    ...
    it('Find missing string in .ts', (done) => {
        ...
        let taskPaths = [];
        fs.readdirSync(tasksPath).forEach((itemName) => {
        ...
        taskPaths.forEach((taskPath) => {
            ...
            let locStringKeysFromJson = [];
            if (taskJson && taskJson.hasOwnProperty('messages')) {
                Object.keys(taskJson.messages).forEach((key) => {
                    locStringKeysFromJson.push(key);
                });
            }
            // find missing keys
            var missingLocStringKeys = [];
            ...
            psFiles.forEach(psFile => {
                ...
                lines.forEach(line => {
                    if (line.search(/Get-VstsLocString/i) > 0) {
                        ...
                        assert(obj.hasOwnProperty('messages') && obj.messages.hasOwnProperty(key), "Loc resource key not found in task.json/module.json. Resource key: '" + key + "', PS file: '" + psFile + "', JSON file: '" + jsonFile + "'.");
                    }
                });
            });
        });
